 ...
 */
int ischa(infoo_tTt *info, char *buf, size_t *p)
{
	size_t j = *p;
 ...
 * Return: Void
 */
void che_cha(infoo_tTt *info, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;
...
int rep_v(infoo_tTt *info)
{
	...
	for (i = 0; info->argv[i]; i++)
	{
		...
		{
			rep_s(&(info->argv[i]),
				  str_duU(co_n(info->status, 10, 0)));
			continue;
		}
		if (!str_mpP(info->argv[i], "$$"))
		{
			rep_s(&(info->argv[i]),
				  str_duU(co_n(getpid(), 10, 0)));
			continue;
		}
		...
		{
			rep_s(&(info->argv[i]),
				  str_duU(str_hr(node->str, '=') + 1));
			continue;
		}
		rep_s(&info->argv[i], str_duU(""));
	}
	return (0);
}
